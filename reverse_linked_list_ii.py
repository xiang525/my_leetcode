...
class Solution(object):
    def reverseBetween(self, head, m, n):
        ...
        dummy = ListNode(0); dummy.next = head; p = head
        m_prev = self.findKth(m,dummy)
        mth = m_prev.next
        nth = self.findKth(n+1,dummy)
        n_next = nth.next
        nth.next = None  # 必须为None否则有两个指向nth.next
